...
def get_checksum(
        url: str, search_pattern: str, algorithm: str
) -> str:
    if algorithm not in ["md5", "sha256", "sha512"]:
        logger.error("Algorithm is not implemented")
        return None
    ...
    for line in checksum_list.splitlines():
        if re.match(search_pattern, line):
            if algorithm == "md5":
                ...
                length = 64
            elif algorithm == "sha516":
                length = 128
            checksum = re.match(f".*([A-Fa-f0-9]{{{length}}}).*", line)
            if checksum.groups():
                return checksum.groups()[0]
